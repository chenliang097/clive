...
import com.rongtuoyouxuan.qfcommon.player.exo.filter.GlPreviewFilter;
...
class EPlayerRenderer extends EFrameBufferObjectRenderer implements SurfaceTexture.OnFrameAvailableListener {
    ...
    private final EPlayerTextureView glPreview;
    private float aspectRatio = 1.0F;
    private SimpleExoPlayer simpleExoPlayer;

    EPlayerRenderer(EPlayerTextureView glPreview) {
    ...
    void setGlFilter(final GlFilter filter) {
        this.glPreview.queueEvent(new Runnable() {
            public void run() {
                ...
                EPlayerRenderer.this.glFilter = filter;
    ...
    public void onSurfaceCreated(EGLConfig config) {
        ...
        (new Handler(this.simpleExoPlayer.getApplicationLooper())).post(new Runnable() {
            public void run() {
                EPlayerRenderer.this.simpleExoPlayer.setVideoSurface(surface);
            }
        });
