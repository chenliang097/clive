package com.rongtuoyouxuan.chatlive.crtutil.tick
...
class Ticker {
    ...
    fun delayNewCountDown(delayMs: Long): TickerJob {
        ...
        val disposable = Observable.timer(delayMs, TimeUnit.MILLISECONDS)
            ...
            .doOnSubscribe {
                com.rongtuoyouxuan.chatlive.crtlog.PLog.d(TAG, "ticker ${System.currentTimeMillis()} start ")
                tickerLiveData.postValue(TickerData(delayMs, delayMs, 0, TickerState.START))
            }
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                com.rongtuoyouxuan.chatlive.crtlog.PLog.d(TAG, "ticker ${System.currentTimeMillis()} next $it")
                tickerLiveData.postValue(TickerData(delayMs, 0, 1, TickerState.TICKER))
            }, {
                it.printStackTrace()
            }, {
                com.rongtuoyouxuan.chatlive.crtlog.PLog.d(TAG, "ticker ${System.currentTimeMillis()} complete ")
