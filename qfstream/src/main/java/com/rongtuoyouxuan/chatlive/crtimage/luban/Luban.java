package com.rongtuoyouxuan.chatlive.crtimage.luban;
...
public class Luban implements Handler.Callback {
    ...
    private File get(InputStreamProvider input, Context context) throws IOException {
        try {
            return new Engine(input, getImageCacheFile(context, Checker.SINGLE.extSuffix(input)), focusAlpha).setMinSide(minSide).compress();
        } finally {
            input.close();
        }
    }
    ...
    private File compressReal(Context context, InputStreamProvider path) throws IOException {
        ...
        if (mCompressionPredicate != null) {
            if (mCompressionPredicate.apply(path.getPath())
                ...
                result = new Engine(path, outFile, focusAlpha).setMinSide(minSide).compress();
            } else {
                result = new File(path.getPath());
            }
        } else {
            result = Checker.SINGLE.needCompress(mLeastCompressSize, path.getPath()) ?
                    new Engine(path, outFile, focusAlpha).setMinSide(minSide).compress() :
