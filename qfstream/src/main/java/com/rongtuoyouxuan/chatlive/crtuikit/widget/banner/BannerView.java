package com.rongtuoyouxuan.chatlive.crtuikit.widget.banner;
...
import com.rongtuoyouxuan.chatlive.crtuikit.widget.banner.adapter.BannerAdapter;
import com.rongtuoyouxuan.chatlive.crtuikit.widget.banner.holder.BannerHolderCreator;
import com.rongtuoyouxuan.chatlive.crtuikit.widget.banner.listener.BannerPointPageChangeListener;
import com.rongtuoyouxuan.chatlive.crtuikit.widget.banner.listener.OnItemClickListener;
import com.rongtuoyouxuan.chatlive.crtuikit.widget.banner.util.ViewPagerSrollerHelper;
...
public class BannerView<T> extends LinearLayout {
    ...
    //延迟循环时间
    private long mDelayTime = 3 * 1000;
    ...
    public BannerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.pl_libgift_BannerView);
        canLoop = attrs.getAttributeBooleanValue(R.styleable.pl_libgift_BannerView_pl_libgift_canLoop, true);
        array.recycle();
        init(context);
    }
    ...
    public BannerView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.pl_libgift_BannerView);
        canLoop = a.getBoolean(R.styleable.pl_libgift_BannerView_pl_libgift_canLoop, true);
        a.recycle();
        init(context);
    }
    ...
    public BannerView clearIndicators() {
        if (ll_dots != null) {
            ll_dots.removeAllViews();
        }
        return this;
     ...
     * @param duration 时长
     */
    public void setSpeedScrollerDuration(int duration) {
        if (mViewPager != null && duration > 0) {
            ViewPagerSrollerHelper.setScroller(getContext(), mViewPager, duration);
        }
    }
    ...
    public BannerView start(long delayTime) {
        if (mViewPager.getAdapter() != null && mViewPager.getAdapter().getRealCount() <= 1) {
            return this;
        }
        if (delayTime <= 0) {
            return this;
...
//        postDelayed(mAutonBannerTask, mDelayTime);
        uiHandler.postDelayed(mAutonBannerTask, mDelayTime);
        return this;
    }
...
//        removeCallbacks(mAutonBannerTask);
    ...
    static class AutoBannerTask implements Runnable {
        ...
        public AutoBannerTask(BannerView bannerView, Handler handler) {
        ...
        public void run() {
            ...
            if (bannerView != null && bannerView.mViewPager != null && bannerView.mViewPager.getAdapter() != null && bannerView.isLoop) {
                int page = bannerView.mViewPager.getCurrentItem() + 1;
                if (page == bannerView.mViewPager.getAdapter().getCount()) {
                    page = 0;
                }
                bannerView.mViewPager.setCurrentItem(page);
//                bannerView.postDelayed(bannerView.mAutonBannerTask, bannerView.mDelayTime);
                if (handler != null) {
                    handler.postDelayed(bannerView.mAutonBannerTask, bannerView.mDelayTime);
                }
            }
        }
    }
    ...
    //控制开始/暂停轮播
    public void setBannerLifecycle(long delayTime, Lifecycle lifecycle) {
