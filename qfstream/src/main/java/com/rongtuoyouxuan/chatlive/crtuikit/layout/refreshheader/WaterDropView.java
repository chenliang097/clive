...
public class WaterDropView extends View {
    ...
    protected final static int BACK_ANIM_DURATION = 200;//回弹动画时长
...
//        mPaint.setShadowLayer(STROKE_WIDTH, STROKE_WIDTH/2, STROKE_WIDTH, 0xffe4e4e4);
        ...
        mPaint.setColor(0xff303151);
        mMaxCircleRadius = DensityUtil.dp2px(8);
        mMinCircleRadius = DensityUtil.dp2px(3);
...
//        updateCompleteState(Math.max(0, 1f * (offset - space) / (maxHeight - space)));
    ...
    public void updateCompleteState(int height) {
        ...
        if (height < space) {
            ...
            //动态设置最下面圆的半径
            bottomCircle.radius = mMaxCircleRadius - (int)(3*y);
