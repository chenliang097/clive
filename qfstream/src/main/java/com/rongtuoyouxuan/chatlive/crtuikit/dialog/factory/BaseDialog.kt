package com.rongtuoyouxuan.chatlive.crtuikit.dialog.factory
...
class BaseDialog constructor(context: Context,
...
) : Dialog(context, config.themeResId) {
    ...
    private fun setWindowLocation() {
        ...
        lp.width = if (config.widthFactor > 0) (com.rongtuoyouxuan.chatlive.crtutil.util.UIUtils.screenWidth(context) * config.widthFactor).toInt()
        else config.widthFactor.toInt()
        lp.height = if (config.heightFactor > 0) (com.rongtuoyouxuan.chatlive.crtutil.util.UIUtils.screenHeight(context) * config.heightFactor).toInt()
