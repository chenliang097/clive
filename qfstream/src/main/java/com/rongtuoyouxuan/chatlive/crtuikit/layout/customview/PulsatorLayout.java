package com.rongtuoyouxuan.chatlive.crtuikit.layout.customview;
...
public class PulsatorLayout extends RelativeLayout {
    ...
    private int WIDTH_RING = DisplayUtil.dipToPixels(getContext(), 5);
    private float mRadiusX = 0.5f;
    private float mScaleXY = 0.7f;
    private int mCount;
    ...
    public PulsatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mEndColor = attr.getColor(R.styleable.Pulsator4Droid_pulse_color_end, DEFAULT_COLOR);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mRadius = Math.min(width, height) * mRadiusX;
        mPaint.setStrokeWidth(WIDTH_RING);
        LinearGradient shader =new LinearGradient(0, height, width, 0, mColor,
    ...
    public void setRingWidth(float dp){
        WIDTH_RING = DisplayUtil.dipToPixels(getContext(), dp);
        mPaint.setStrokeWidth(WIDTH_RING);
        clear();
        build();
    }
    ...
    private void build() {
        ...
        for (int index = 0; index < mCount; index++) {
            ...
            pulseView.setScaleX(mScaleXY);
            pulseView.setScaleY(mScaleXY);
            ...
            // setup animators
            ObjectAnimator scaleXAnimator = ObjectAnimator.ofFloat(pulseView, "ScaleX", mScaleXY, 1.0f);
            scaleXAnimator.setRepeatCount(repeatCount);
            scaleXAnimator.setRepeatMode(ObjectAnimator.RESTART);
            scaleXAnimator.setStartDelay(delay);
            animators.add(scaleXAnimator);

            ObjectAnimator scaleYAnimator = ObjectAnimator.ofFloat(pulseView, "ScaleY", mScaleXY, 1.0f);
            scaleYAnimator.setRepeatCount(repeatCount);
            scaleYAnimator.setRepeatMode(ObjectAnimator.RESTART);
            scaleYAnimator.setStartDelay(delay);
            animators.add(scaleYAnimator);

            ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(pulseView, "Alpha", 1f, 0.0f);
...
//    }
    ...
    private class PulseView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            canvas.drawCircle(mCenterX, mCenterY, mRadius - WIDTH_RING, mPaint);
        }
