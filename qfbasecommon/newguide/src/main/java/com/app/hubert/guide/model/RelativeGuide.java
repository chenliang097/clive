...
public class RelativeGuide {
     ...
     * @param gravity 仅限left top right bottom
     * @param padding 与高亮view的padding，单位px
     * @param offestX 只有在 top bottom 的时候才会起作用以 相对View 的 left 为基准
     */
    public RelativeGuide(@LayoutRes int layout, @LimitGravity int gravity, int padding, int offestX) {
        this.layout = layout;
        this.gravity = gravity;
        this.padding = padding;
        this.leftOffest = offestX;
    }

    public View getGuideLayout(ViewGroup viewGroup, Controller controller) {
    ...
    private MarginInfo getMarginInfo(@LimitGravity int gravity, ViewGroup viewGroup, View view) {
        ...
        switch (gravity) {
            ...
            case Gravity.TOP:
                ...
                marginInfo.leftMargin = (int) rectF.left + leftOffest;
                break;
            case Gravity.RIGHT:
